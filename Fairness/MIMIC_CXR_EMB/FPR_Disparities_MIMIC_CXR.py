...
def get_Sex_FPR_Disparities(basepath, diseases, diseases_abbr, number_of_runs, significance_level,
    ...
    seed_19_sex = pd.read_csv(
        basepath+"Run_seed19_FPR_GAP_sex.csv", index_col=0)
    seed_31_sex = pd.read_csv(
        basepath+"Run_seed31_FPR_GAP_sex.csv", index_col=0)
    seed_38_sex = pd.read_csv(
        basepath+"Run_seed38_FPR_GAP_sex.csv", index_col=0)
    seed_47_sex = pd.read_csv(
        basepath+"Run_seed47_FPR_GAP_sex.csv", index_col=0)
    seed_77_sex = pd.read_csv(
        basepath+"Run_seed77_FPR_GAP_sex.csv", index_col=0)
    ...
    sex_fnr_disp_df = pd.DataFrame(d)
    sex_fnr_disp_df = sex_fnr_disp_df.sort_values(by='Distance')

    sex_fnr_disp_df.to_csv(basepath+"sex_fpr_disp.csv", index=False)

    # Plotting the results
    plot_Sex_FPR_GAPS(basepath, sex_fnr_disp_df, height,
                      font_size, rotation_degree)


def plot_Sex_FPR_GAPS(basepath, sex_fnr_disp_df,
                      height, font_size, rotation_degree):
    ...
    plt.scatter(sex_fnr_disp_df['diseases_abbr'], sex_fnr_disp_df['Gap_M_mean'],
                s=np.multiply(sex_fnr_disp_df['#M'], 0.05), marker='o', color='blue', label="Male")
    plt.errorbar(sex_fnr_disp_df['diseases_abbr'], sex_fnr_disp_df['Gap_M_mean'],
                 yerr=sex_fnr_disp_df['CI_M'], fmt='o', mfc='blue')
    plt.scatter(sex_fnr_disp_df['diseases_abbr'], sex_fnr_disp_df['Gap_F_mean'],
                s=np.multiply(sex_fnr_disp_df['#F'], 0.05), marker='o', color='red', label="Female")
    plt.errorbar(sex_fnr_disp_df['diseases_abbr'], sex_fnr_disp_df['Gap_F_mean'],
                 yerr=sex_fnr_disp_df['CI_F'], fmt='o', mfc='red')
...
def get_Age_FPR_Disparities(basepath, diseases, diseases_abbr, number_of_runs, significance_level,
    ...
    seed_19_age = pd.read_csv(
        basepath+"Run_seed19_FPR_GAP_Age.csv", index_col=0)
    seed_31_age = pd.read_csv(
        basepath+"Run_seed31_FPR_GAP_Age.csv", index_col=0)
    seed_38_age = pd.read_csv(
        basepath+"Run_seed38_FPR_GAP_Age.csv", index_col=0)
    seed_47_age = pd.read_csv(
        basepath+"Run_seed47_FPR_GAP_Age.csv", index_col=0)
    seed_77_age = pd.read_csv(
        basepath+"Run_seed77_FPR_GAP_Age.csv", index_col=0)
    ...
    age_fnr_disp_df.to_csv(basepath+"age_fpr_disp.csv", index=False)

    # Plotting the results
    plot_age_FPR_GAPS(basepath, age_fnr_disp_df, height,
                      font_size, rotation_degree)


def plot_age_FPR_GAPS(basepath, age_fnr_disp_df, height, font_size, rotation_degree):
    ...
    plt.errorbar(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_60-80_mean'],
                 yerr=age_fnr_disp_df['CI_60-80'], fmt='o', mfc='blue')
    plt.scatter(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_40-60_mean'],
                s=np.multiply(age_fnr_disp_df['#40-60'], 0.5), marker='o', color='orange', label="40-60")
    plt.errorbar(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_40-60_mean'],
                 yerr=age_fnr_disp_df['CI_40-60'], fmt='o', mfc='orange')
    plt.scatter(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_20-40_mean'],
                s=np.multiply(age_fnr_disp_df['#20-40'], 0.5), marker='o', color='green', label="20-40")
    plt.errorbar(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_20-40_mean'],
                 yerr=age_fnr_disp_df['CI_20-40'], fmt='o', mfc='green')
    plt.scatter(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_80-_mean'],
                s=np.multiply(age_fnr_disp_df['#80-'], 0.5), marker='o', color='red', label="80-")
    plt.errorbar(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_80-_mean'],
                 yerr=age_fnr_disp_df['CI_80-'], fmt='o', mfc='red')
    plt.scatter(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_0-20_mean'],
                s=np.multiply(age_fnr_disp_df['#0-20'], 0.5), marker='o', color='purple', label="0-20")
    plt.errorbar(age_fnr_disp_df['diseases_abbr'], age_fnr_disp_df['Gap_0-20_mean'],
                 yerr=age_fnr_disp_df['CI_0-20'], fmt='o', mfc='purple')
...
def get_Race_FPR_Disparities(basepath, diseases, diseases_abbr, number_of_runs, significance_level,
    ...
    seed_19_race = pd.read_csv(basepath +
                               "Run_seed19_FPR_GAP_race.csv", index_col=0)
    seed_31_race = pd.read_csv(basepath +
                               "Run_seed31_FPR_GAP_race.csv", index_col=0)
    seed_38_race = pd.read_csv(
        basepath + "Run_seed38_FPR_GAP_race.csv", index_col=0)
    seed_47_race = pd.read_csv(
        basepath + "Run_seed47_FPR_GAP_race.csv", index_col=0)

    seed_77_race = pd.read_csv(
        basepath + "Run_seed77_FPR_GAP_race.csv", index_col=0)
    ...
    race_fnr_disp_df = pd.DataFrame(d)
    race_fnr_disp_df = race_fnr_disp_df.sort_values(by='Distance')
    race_fnr_disp_df.to_csv(basepath+"race_fpr_disp.csv", index=False)

    # Plotting the results
    # plot_race_FPR_GAPS(basepath, race_fnr_disp_df, height,
    #                    font_size, rotation_degree)


def plot_race_FPR_GAPS(basepath, race_fnr_disp_df, height, font_size, rotation_degree):

    plt.ioff()
    plt.rcParams.update({'font.size': font_size})
    plt.figure(figsize=(16, height))

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_W_mean'],
                s=np.multiply(race_fnr_disp_df['#White'], 0.095), marker='o', color='blue', label="WHITE")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_W_mean'],
                 yerr=race_fnr_disp_df['CI_W'], fmt='o', mfc='blue')  # ecolor='blue'

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_B_mean'],
                s=np.multiply(race_fnr_disp_df['#Black'], 0.095), marker='o', color='orange', label="BLACK")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_B_mean'],
                 yerr=race_fnr_disp_df['CI_B'], fmt='o', mfc='orange')

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_H_mean'],
                s=np.multiply(race_fnr_disp_df['#Hisp'], 0.095), marker='o', color='green', label="HISPANIC")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_H_mean'],
                 yerr=race_fnr_disp_df['CI_H'], fmt='o', mfc='green')

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_Ot_mean'],
                s=np.multiply(race_fnr_disp_df['#Other'], 0.095), marker='o', color='r', label="OTHER")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_Ot_mean'],
                 yerr=race_fnr_disp_df['CI_Ot'], fmt='o', mfc='r')

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_As_mean'],
                s=np.multiply(race_fnr_disp_df['#Asian'], 0.095), marker='o', color='m', label="ASIAN")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_As_mean'],
                 yerr=race_fnr_disp_df['CI_As'], fmt='o', mfc='m')

    plt.scatter(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_Am_mean'],
                s=np.multiply(race_fnr_disp_df['#American'], 0.095), marker='o', color='k', label="NATIVE")
    plt.errorbar(race_fnr_disp_df['diseases_abbr'], race_fnr_disp_df['Gap_Am_mean'],
                 yerr=race_fnr_disp_df['CI_Am'], fmt='o', mfc='k')
...
def get_Insurance_FPR_Disparities(basepath, diseases, diseases_abbr, number_of_runs, significance_level,
    ...
    seed_19_insurance = pd.read_csv(
        basepath+"Run_seed19_FPR_GAP_insurance.csv", index_col=0)
    seed_31_insurance = pd.read_csv(
        basepath+"Run_seed31_FPR_GAP_insurance.csv", index_col=0)
    seed_38_insurance = pd.read_csv(
        basepath+"Run_seed38_FPR_GAP_insurance.csv", index_col=0)
    seed_47_insurance = pd.read_csv(
        basepath+"Run_seed47_FPR_GAP_insurance.csv", index_col=0)
    seed_77_insurance = pd.read_csv(
        basepath+"Run_seed77_FPR_GAP_insurance.csv", index_col=0)
    ...
    insurance_fnr_disp_df = pd.DataFrame(d)
    insurance_fnr_disp_df = insurance_fnr_disp_df.sort_values(by='Distance')
    insurance_fnr_disp_df.to_csv(
        basepath+"insurance_fpr_disp.csv", index=False)

    # Plotting the results
    plot_insurance_FPR_GAPS(basepath, insurance_fnr_disp_df, height,
                            font_size, rotation_degree)


def plot_insurance_FPR_GAPS(basepath, insurance_fnr_disp_df, height, font_size, rotation_degree):

    plt.ioff()
    plt.rcParams.update({'font.size': font_size})
    plt.figure(figsize=(16, height))
    plt.scatter(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_C_mean'],
                s=np.multiply(insurance_fnr_disp_df['#Medicare'], 0.095), marker='o', color='blue', label="MEDICARE")
    plt.errorbar(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_C_mean'],
                 yerr=insurance_fnr_disp_df['CI_C'], fmt='o', mfc='blue')

    plt.scatter(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_O_mean'],
                s=np.multiply(insurance_fnr_disp_df['#Other'], 0.095), marker='o', color='orange', label="OTHER")
    plt.errorbar(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_O_mean'],
                 yerr=insurance_fnr_disp_df['CI_O'], fmt='o', mfc='orange')

    plt.scatter(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_A_mean'],
                s=np.multiply(insurance_fnr_disp_df['#Medicaid'], 0.095), marker='o', color='green', label="MEDICAID")
    plt.errorbar(insurance_fnr_disp_df['diseases_abbr'], insurance_fnr_disp_df['Gap_A_mean'],
                 yerr=insurance_fnr_disp_df['CI_A'], fmt='o', mfc='green')
